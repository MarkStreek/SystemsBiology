...
initial.values <- c(DR_N = 0, mRNAr = 4.74, R = 267, DR = 53e-9 * 267)
```
...
verandering_dt <- function (time, parameters, initial.values) {
  with(as.list(c(parameters, initial.values)),{
    # Vergelijking van R
    R <- Ksr * mRNAr + RF * Kre * DR_N - Kon * D * R - Kd_R * R
    # Vergelijking van mRNAr
    mRNAr <- Ks_Rm * (1 - (DR_N / IC50_Rm)) - Kd_Rm * mRNAr
    # Vergelijking van DR
    DR <- Kon * D * R - KT * D * R
    # Vergelijking van DR(N)
    DR_N <- KT * (D * R) - Kre * DR_N
    
    # Retourneren van de initiële waardes
    return(list(c(DR_N, mRNAr, R, DR)))
  })
}
