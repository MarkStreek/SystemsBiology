...
Bij de simulatie scenario's wordt er gekeken naar het verloop van het systeem, als de toevoeging van D stopt na het bereiken van de steady state. Onderstaand de functie die hierbij gebruikt wordt.
...
plot(verloop.dataset.lage.doses[,"mRNAr"],main = "mRNAr met lage dosis",type="l", xlab="Time", ylab="mRNAr", lwd = 2, xlim = c(0, 150), ylim = c(0, 8), col = "#21A9AD")
# Toevoegen experementiele mediaan lage dosis
lines(medians$mRNA[medians$dose == 0.1] ~ medians$time[medians$dose == 0.1],type = "l", lwd = 2, col = "darkred")
# Toevoegen losse experiment punten
points(experiment.data$mRNA, col = "black", pch = 1, cex = .7)

# mRNA hoge dosis

# Plotten van het model
plot(verloop.dataset.hoge.doses[,"mRNAr"], main = "mRNAr met hoge dosis",type="l", xlab="Time", ylab="Free receptor", lwd = 2, xlim = c(0, 150), ylim = c(0, 8), col = "#21A9AD")
# Toevoegen experementiele mediaan hoge dosis
lines(medians$mRNA[medians$dose == 0.3] ~ medians$time[medians$dose == 0.3], type = "l", lwd = 2, col = "darkred")
# Toevoegen losse experiment punten
points(experiment.data$mRNA, pch = 1, cex = .7, col = "black")

# Vrije R lage dosis

# Plotten van het model
plot(verloop.dataset.lage.doses[,"R"],main = "Free receptor (R) met lage dosis",type="l", xlab="Time", ylab="Free receptor", lwd = 2, xlim = c(0, 140), ylim = c(0, 500), col = "#21A9AD")
# Toevoegen experementiele mediaan lage dosis
...
# Plotten van het model
plot(verloop.dataset.hoge.doses[,"R"],main = "Free receptor (R) met hoge dosis",type="l", xlab="Time", ylab="Free receptor", lwd = 2, xlim = c(0, 140), ylim = c(0, 500), col = "#21A9AD")
# Toevoegen experementiele mediaan hoge dosis
lines(medians$Free_receptor[medians$dose == 0.3] ~ medians$time[medians$dose == 0.3], type = "l", lwd = 2, col = "darkred")
# Toevoegen losse experiment punten
points(experiment.data$Free_receptor, col = "black", pch = 1, cex = .7)

```

```{r, fig.cap="**Hier moet de omschrijving van het figuur komen.**"}
library(ggplot2)
library(patchwork)

data <- data.frame(
  time = verloop.dataset.lage.doses[, 1],
  waarde = verloop.dataset.lage.doses[, 3])

data2 <- data.frame(
  time = medians$time[medians$dose == 0.1],
  waarde = medians$mRNA[medians$dose == 0.1])

plot1 <- ggplot(mapping = aes(x = time, y = waarde)) +
  geom_line(data = data, aes(colour = "Model"), size = 1.5) +
  geom_line(data = data2, aes(colour = "Experimentele data"), size = 1.5) +
  scale_colour_manual(values = c("darkred", "steelblue")) + ylim(0,5)

data3 <- data.frame(
  time = verloop.dataset.hoge.doses[, 1],
  waarde = verloop.dataset.hoge.doses[, 3]
)

data4 <- data.frame(
  time = medians$time[medians$dose == 0.3],
  waarde = medians$mRNA[medians$dose == 0.3]
)

plot2 <- ggplot(mapping = aes(x = time, y = waarde)) +
  geom_line(data = data3, aes(colour = "Model"), size = 1.5) +
  geom_line(data = data4, aes(colour = "Experiment"), size = 1.5) +
  scale_colour_manual(values = c("darkred", "steelblue")) + ylim(0,5)
...
plot(verloop.dataset.steady[,"DR_N"], 
     type = "l", main = "Figuur C",
     xlab = "Tijd (in uren)",
     ylab = "Hoeveelheid DR_N (n)")

plot(verloop.dataset.steady[,"mRNAr"],
     type = "l", main = "Figuur D",
     xlab = "Tijd (in uren)",
     ylab = "Hoeveelheid mRNAr (n)")
