...
```{r, fig.cap="*Nog invullen*"}
verloop.dataset.steady <- ode(initial.values.1, times.2.2, 
                              verandering_dt.2.2, parameters.1, D=53.4)

verloop.dataset.96 <- ode(initial.values.1, times.2.2, 
                              verandering_dt.1, parameters.1, D=53.4)


steady.state <- data.frame(
  time = verloop.dataset.steady[, 1],
  DR_N = verloop.dataset.steady[, 2],
  mRNAr = verloop.dataset.steady[, 3]
)

normaal.verloop <- data.frame (
  time = verloop.dataset.96[, 1],
  DR_N = verloop.dataset.96[, 2],
  mRNAr = verloop.dataset.96[, 3]
)

ggplot() +
  geom_line(data = normaal.verloop, mapping = aes(x = time, y = mRNAr, colour = "mRNAr"), linewidth = 1.2) +
  geom_line(data = normaal.verloop, mapping = aes(x = time, y = DR_N, colour = "DR_NR"), linewidth = 1.2) +
  scale_colour_manual(values = c("darkred", "steelblue")) +
  labs(title = "Verloop van [mRNAr] en [DR_N]", subtitle = "Normale situatie") +
  theme(plot.title = element_text(color="black", size=12, face="bold.italic"),
        plot.subtitle = element_text(color = "red", size = 10))

ggplot() +
  geom_line(data = steady.state, mapping = aes(x = time, y = mRNAr, colour = "mRNAr"), linewidth = 1.2) +
  geom_line(data = steady.state, mapping = aes(x = time, y = DR_N, colour = "DR_NR"), linewidth = 1.2) +
  scale_colour_manual(values = c("darkred", "steelblue")) +
  labs(title = "Verloop van [mRNAr] en [DR_N]", subtitle = "Toedienen van D stopt na bereiking van de steady state") +
  theme(plot.title = element_text(color="black", size=12, face="bold.italic"),
        plot.subtitle = element_text(color = "red", size = 10))
```
