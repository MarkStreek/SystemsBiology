...
mRNAr_lage.dosis <- ggplot(mapping = aes(x = time, y = mRNAr_waarde)) +
  ...
  geom_line(data = experiment.lage.dosis, aes(colour = "Experimentele data"), 
...
R_hoge.dosis <- ggplot(mapping = aes(x = time, y = R_waarde)) +
  geom_line(data = hoge.dosis, aes(colour = "Model"), linewidth = 1.5) +
  geom_line(data = experiment.hoge.dosis, aes(colour = "Experimentele data"), 
  linewidth = 1.5) + scale_colour_manual(values = c("darkred", "steelblue")) + 
...
```{r, fig.cap = "***Model verloop samen met experimentele data. Het plot bestaat uit vier kleine plotjes. Bij alle figuren staat de tijd op de x-as. Bij de bovenste twee figuren staat mRNAr op de y-as en bij de onderste twee figuren staat het aantal vrije receptoren op de y-as. Er wordt gekeken naar het verloop van het model samen met de experimentele data bij zowel een lage, als hoge dosis D. Voor de toediening van een lage en hoge dosis, is er weinig effect te zien bij het model-verloop van mRNAr. Bij een hoge dosis is het aantal model-mRNAr een fractie lager. Bij de experimentele dat is er wel een duidelijk verschil tussen lage en hoge dosis. Bij een lage dosis D is er te zien dat het verloop van het mRNAr een stuk schokkeriger is: de punten liggen verder uit elkaar. D.w.z., er zijn meer pieken en dalen. Beide dosissen vertonen wel een vergelijkbaar patroon, eerste blijf het verloop een rechte lijn, vervolgens daalt het iets en tot slot stijgt het explosief omhoog. Voor het aantal vrije receptoren is er wel een opvallend verschil bij de model-data. Bij een hoger dosis wordt de steady state sneller bereikt en ligt het niveau van de steady state een stuk lager. Voor de experimentele data ligt het niveau bij een lage dosis een fractie boven de model waarde. Voor een hoge dosis ligt de experimentele data ongeveer op een gelijk niveau, maar het daalt na verloop van tijd wel lineair.***"}
...
De verwachting voor simulatie (4.2.3) kwam overeen met de resultaten van $mRNAr$, bij zowel $Kon$ als $Kre$. Een deling van vijf zorgde voor een sterkere afname dan door twee. Verder zorgde de product waarde van vijf voor een hogere aanmaak dan twee. Het bevestigen van deze verwachtigingen, toont aan dat $Kon$ en $Kre$ een regulerende invloed hebben op de aanmaak van $mRNAr$.
