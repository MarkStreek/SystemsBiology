...
$$\frac{df}{dt} = - \overbrace{{\eta}_f mf}^{afbraak} - \overbrace{l_{1}f}^{f{→}TSG{_1}^{+/-}}$$
...
$$\frac{dq}{dt} = \overbrace{{\lambda}_{q}q(1 - A)}^{proliferatie} - \overbrace{{\eta}_{q}nq}^{celdood}  + \overbrace{l_{1}f}^{f→TSG_{1}^{+/-}} - \overbrace{{\rho}_{2}q}^{TSG1_{1}^{+/-} →TSG_{1}^{-/-}}$$  
...
$$\frac{dr}{dt} = \overbrace{{\lambda}_{q}r(1 - A)}^{proliferatie} - \overbrace{{\eta}_{r}nr}^{celdood}  + \overbrace{{\rho}_{2}q}^{TSG_{1}^{+/-}→TSG_{1}^{-/-}} - \overbrace{{\rho}_{3}r}^{TSG1_{1}^{-/-} →TSG_{2}^{+/-}}$$ 
...
$$\frac{ds}{dt} = \overbrace{{\lambda}_{s}s(1 - A)}^{proliferatie} - \overbrace{{\eta}_{r}ns}^{celdood}  + \overbrace{{\rho}_{3}r}^{TSG1_{1}^{-/-} →TSG_{2}^{+/-}} - \overbrace{{\rho}_{4}s}^{TSG_{2}^{+/-} →n}$$
...
In dit hoofdstuk worden alle variabelen beschreven uit het biologische model. Deze data is genormaliseerd door de parameters $f, q, r, s, n, m, x, t$ te delen door hun beginwaardes e.g. $(\frac{f}{f_0})$. 
